...
pub fn load_default_rsa_jwks(url: String, algorithm: Algorithm) -> Vec<Box<dyn TokenDecoder<DefaultJwt>>> {
    ...
    for jwk in jwk_loader.jwks.keys {
        jwk_decoders.push(Box::new(RsaJwtDecoder::new(algorithm, vec![
            Box::new(jwk)
        ])));
    }
    jwk_decoders
