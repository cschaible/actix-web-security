...
static AUTH_ERROR_MESSAGE_MAPPING: Lazy<HashMap<AuthenticationError, String>> = Lazy::new(|| {
    ...
    add_env_error_message(AuthenticationError::InvalidAuthentication, "invalid authentication type".to_string(), &mut error_messages);
    ...
    add_env_error_message(AuthenticationError::AuthorizationHeaderNotSet, "authorization header not set".to_string(), &mut error_messages);
    error_messages
});
static AUTH_ERROR_CONTENT_TYPE: Lazy<String> = Lazy::new(|| {
    match std::env::var("AUTH_ERROR_CONTENT_TYPE") {
        Ok(content_type) => content_type,
        _ => "text/html; charset=utf-8".to_string()
    }
});

fn add_env_error_code(error: AuthenticationError, error_codes: &mut HashMap<AuthenticationError, u16>) {
    match std::env::var(format!("{}_code", error.to_string())) {
        Ok(code) => {
            error_codes.insert(error, code.parse::<u16>().expect("Invalid status code mapping"))
        }
        _ => error_codes.insert(error, 401)
    };
}

fn add_env_error_message(error: AuthenticationError, default_message: String, error_messages: &mut HashMap<AuthenticationError, String>) {
...
impl error::ResponseError for AuthenticationError {
    fn status_code(&self) -> StatusCode {
        match *self {
            AuthenticationError::InvalidAuthentication => dynamic_status_code(&AuthenticationError::InvalidAuthentication),
            AuthenticationError::AuthorizationHeaderNotSet => dynamic_status_code(&AuthenticationError::AuthorizationHeaderNotSet),
            AuthenticationError::InvalidAuthorizationHeader => dynamic_status_code(&AuthenticationError::InvalidAuthorizationHeader),
            AuthenticationError::UsernameNotFound => dynamic_status_code(&AuthenticationError::UsernameNotFound),
            AuthenticationError::InvalidToken => dynamic_status_code(&AuthenticationError::InvalidToken),
        }
    }
...
fn dynamic_status_code(error: &AuthenticationError) -> StatusCode {
    StatusCode::from_u16(*AUTH_ERROR_STATUS_CODE_MAPPING.get(error).expect("Status code mapping missing")).expect("Invalid status code mapping found")
}
